...
def sigma(corr, rho_mean, r_i, r_x):
	...
	c = scipy.interpolate.UnivariateSpline(np.log(r_i), np.log(corr), s=0)#0.0032)
	...
	for i in range(len(r_x)):
		...
		integ =  lambda x: (1.0 + np.exp(c(np.log(r_x[i]/x)))) / ((x**2.0) * ((1.0 - (x**2.0))**0.5)) # for int from 0 to 1
